...
intersect_plane(ray const& r,
    ...
    glm::vec3 a1 = p1 - p0;
    glm::vec3 a2 = p2 - p0;
    glm::vec3 n = (glm::cross(a1,a2)/glm::length(glm::cross(a1,a2)));
    //float d = glm::dot(a1,n);
    float d = glm::dot(p0, n);
    if(glm::dot(n,r.direction) == 0) return false;
    float t = (d-glm::dot(n,r.origin))/(glm::dot(n,r.direction));
...
edge_test(glm::vec3 const& p0,
    ...
    glm::vec3 a1 = p1-p0;
    glm::vec3 a2 = x-p0;
    glm::vec3 n_test = glm::cross(a1,a2);
    float scalar = glm::dot(n_test,face_normal);
...
intersect_triangle(ray const& r,
    ...
    float b0 = glm::length(glm::cross(p1-x,p2-x))/glm::length(glm::cross(p1-p0,p2-p0));
    float b1 = glm::length(glm::cross(x-p0,p2-p0))/glm::length(glm::cross(p1-p0,p2-p0));
    float b2 = glm::length(glm::cross(p1-p0,x-p0))/glm::length(glm::cross(p1-p0,p2-p0));
