...
void main()
{
    ...
    spec = pow(max(dot(normal, halfwayDir), 0.0), 20.0) * smoothness * spotlight_activ;
    ...
    vec3 lighting = ((ambient + diffuse + specular) * dist * 30 * rand_light * color + spotlight * 200 * pow(dist_flash,1.5) * vec3(0.6f,0.6f,0.7f) * color + spec * color);//;
